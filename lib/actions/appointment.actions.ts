...
export const getRecentAppointment = async () => {
  try {
    ...
    const scheduledAppointments = (appointments.documents as Appointment[]).filter((appointment) => appointment.status === "scheduled");

    const pendingAppointments = (appointments.documents as Appointment[]).filter((appointment) => appointment.status === "pending");

    const cancelledAppointments = (appointments.documents as Appointment[]).filter((appointment) => appointment.status === "cancelled");
...
export const updateAppointment = async ({ appointmentId, appointment, type, userId, timeZone }: UpdateAppointmentParams) => {
  try {
    ...
    const smsMessage: string = `Greetings from CarePulse. ${
      type === "schedule"
